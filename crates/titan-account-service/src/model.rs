...
use crate::{
    ...
    DirectCancelOrderRequest, DirectExecutionError, DirectNewOrderRequest, ExecutionFuture,
...
pub struct AccountEventPublisher {
    ...
    sink: Option<Arc<dyn AccountEventSink>>,
...
/// Service-side event backend for an in-process account connector. The concrete
...
impl AccountEventPublisher {
    ...
    fn validate_header(&self, header: AccountEventHeaderV1) -> Result<(), CoreError> {
...
fn publisher_error(message: impl Into<Arc<str>>) -> CoreError {
    CoreError::new(
        titan_core_types::ErrorKind::SubscriptionRejected,
        titan_core_types::ComponentIdentity::new("titan.account", "publisher"),
...
pub trait AccountConnector: Send + Sync + 'static {
    ...
    /// Returns an immediately-drivable REST future with no per-account queue, journal, retry or
    /// bootstrap side effects.
    fn direct_submit(&self, _request: DirectNewOrderRequest) -> ExecutionFuture {
        Box::pin(async {
            Err(DirectExecutionError {
                exchange: "unavailable",
                code: "DIRECT_EXECUTION_UNAVAILABLE".into(),
                message: "connector does not expose direct execution".into(),
                outcome_unknown: false,
            })
        })
    }

    fn direct_cancel(&self, _request: DirectCancelOrderRequest) -> ExecutionFuture {
        Box::pin(async {
            Err(DirectExecutionError {
                exchange: "unavailable",
                code: "DIRECT_EXECUTION_UNAVAILABLE".into(),
