...
use hftbacktest::{
    backtest::{
        Backtest,
        bar::{BarFeed, MaterializedBarFeed},
        platform::{
    ...
    prelude::{Bot, ElapseResult, Event, Order},
    types::{Side, Status},
};
...
pub struct TickFrameSource<'a, B, MD> {
    ...
    captured_funding_reports: Vec<hftbacktest::backtest::execution::FundingReport>,
    order_pending: bool,
    fill_pending: bool,
...
pub enum TickRuntimeError<E: std::error::Error + Send + Sync + 'static> {
    #[error(transparent)]
    Bot(E),
...
{
    pub fn new(hbt: &'a mut B, frame_interval: i64, max_tick_batch: usize) -> Self {
        ...
        let mut source = Self {
            ...
            captured_funding_reports: Vec::new(),
            order_pending: false,
            fill_pending: false,
    ...
    fn execution_command_to_abi(
        ...
        Ok(match command {
            ExecutionCommand::Submit(request) => OrderCommand {
                kind: crate::ORDER_COMMAND_SUBMIT,
            ...
            ExecutionCommand::Cancel(request) => OrderCommand {
                kind: crate::ORDER_COMMAND_CANCEL,
                asset_no,
                order_id: request.client_order_id,
    ...
    pub fn account_snapshots(
        ...
        self.hbt.runtime_account_snapshots()
    }
    ...
    fn process_funding_boundary(&mut self) -> Result<bool, TickRuntimeError<B::Error>> {
        ...
        if let Some(report) = self
            ...
            .map_err(TickRuntimeError::Bot)?
        {
            self.captured_funding_reports.push(report);
...
pub struct HybridFrameSource<'a, B, MD> {
...
#[derive(Debug, thiserror::Error)]
pub enum HybridRuntimeError<E: std::error::Error + Send + Sync + 'static> {
    #[error(transparent)]
    ...
    B::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    pub fn execution_reports(&self) -> &[hftbacktest::backtest::execution::ExecutionReport] {
        self.tick.execution_reports()
    }

    pub fn funding_reports(&self) -> &[hftbacktest::backtest::execution::FundingReport] {
        self.tick.funding_reports()
    }
    ...
    pub fn account_snapshots(
        ...
        self.tick.account_snapshots()
    }

    pub fn schedule_timer(&mut self, timer: RuntimeTimer) {
        self.tick.schedule_timer(timer);
    }
    ...
    pub fn schedule_funding(
        ...
        self.tick.schedule_funding(funding)
    }
