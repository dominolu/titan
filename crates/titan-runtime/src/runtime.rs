//! Titan's single Rust-owned event runtime with a stable, extensible C callback ABI.
//!
...
use hftbacktest::{
    ...
    market_data::{BAR_EMPTY, BarHistory},
};
...
/// Decode the transport-only ABI object exactly once at the engine boundary.
pub fn decode_order_command(
    command: OrderCommand,
    now: i64,
    venue_id: VenueId,
    instrument_id: InstrumentId,
) -> Result<Option<hftbacktest::backtest::execution::ExecutionCommand>, OrderCommandDecodeError> {
    ...
    match command.kind {
        ...
        ORDER_COMMAND_SUBMIT => {
            let origin = match command._reserved[2] {
            ...
            let side = match command.side {
                1 => hftbacktest::types::Side::Buy,
                -1 => hftbacktest::types::Side::Sell,
                _ => return Err(OrderCommandDecodeError::InvalidSide),
            };
            let time_in_force = match command.time_in_force {
                0 => hftbacktest::types::TimeInForce::GTC,
                1 => hftbacktest::types::TimeInForce::GTX,
                2 => hftbacktest::types::TimeInForce::FOK,
                3 => hftbacktest::types::TimeInForce::IOC,
                _ => return Err(OrderCommandDecodeError::InvalidTimeInForce),
            };
            let order_type = match command.order_type {
                0 => hftbacktest::types::OrdType::Limit,
                1 => hftbacktest::types::OrdType::Market,
            ...
            if !command.price.is_finite()
                || (order_type == hftbacktest::types::OrdType::Limit && command.price <= 0.0)
            {
            ...
            Ok(Some(ExecutionCommand::Submit(ExecutionOrderRequest {
                ...
                time_in_force,
                order_type,
                reduce_only: command._reserved[0] != 0,
                origin,
                local_submit_ts: now,
            })))
        }
...
/// Transitional engine adapter for callers that previously decoded commands as an
/// inherent ABI method. It disappears with the old Python Runtime driver.
pub trait OrderCommandExt {
    fn decode_execution(
        self,
        now: i64,
        venue_id: VenueId,
...
impl OrderCommandExt for OrderCommand {
    fn decode_execution(
        self,
        now: i64,
        venue_id: VenueId,
    ...
    {
        decode_order_command(self, now, venue_id, instrument_id)
    }
}

pub trait RuntimeFundingExt: Sized {
    fn config(self) -> Result<FundingConfig, MaterializedBarError>;
    fn from_report(
        asset_no: u32,
...
impl RuntimeFundingExt for RuntimeFunding {
    fn config(self) -> Result<FundingConfig, MaterializedBarError> {
    ...
    fn from_report(
        ...
        let formula = match config.formula {
            hftbacktest::backtest::execution::FundingFormula::InstrumentNotional => 0,
        };
        Self {
...
impl CallbackRegistry {
    ...
    /// Builds a registry from process-local native callback addresses produced by the
    /// strategy compiler. A zero address means that the event has no handler.
    ///
    /// # Safety
    ///
    /// Every non-zero address must remain executable for the full lifetime of all runtime
    /// calls using the returned registry and must implement [`StrategyCallback`]'s exact ABI.
    pub unsafe fn from_addresses(
        addresses: &[usize; EVENT_SLOT_COUNT],
    ) -> Result<Self, RuntimeError> {
        let mut registry = Self::default();
        for (event_id, address) in addresses.iter().copied().enumerate() {
            if address == 0 {
                continue;
            }
            // Safety: upheld by the caller; converting a code address to the declared native
            // callback type is the only unsafe boundary between the compiler and Runtime.
            let callback: StrategyCallback = unsafe { std::mem::transmute(address) };
            registry.set_custom(event_id as u32, callback)?;
        }
        Ok(registry)
    }
...
pub trait RuntimeEventSource {
    ...
    fn classify_error(
        ...
        (
            hftbacktest::backtest::result::EngineComponent::DataSource,
            hftbacktest::backtest::result::EngineErrorCode::InvalidData,
        )
    }
}
...
fn action_precedes_matching(
    key: hftbacktest::backtest::scheduler::EventKey,
    exchange_ts: i64,
) -> bool {
...
pub struct MaterializedBarSource {
    ...
    action_scheduler: hftbacktest::backtest::scheduler::GlobalScheduler<PendingBarAction>,
    ...
    timers: TimerQueue,
    scheduler: hftbacktest::backtest::scheduler::GlobalScheduler<BarBoundary>,
    ...
    end_policy: hftbacktest::backtest::result::EndPolicy,
    data_end_ts: i64,
}
...
impl MaterializedBarSource {
    pub fn new(
        ...
        Ok(Self {
            ...
            feed_latency_ns: 0,
            entry_latency_ns: 0,
            action_scheduler: hftbacktest::backtest::scheduler::GlobalScheduler::new(),
            ...
            scheduler: hftbacktest::backtest::scheduler::GlobalScheduler::new(),
            ...
            end_policy: hftbacktest::backtest::result::EndPolicy::DrainAll,
            data_end_ts,
        })
    }
    ...
    fn next_boundary(
        ...
        use hftbacktest::backtest::scheduler::EventPhase;
        ...
        let limit = match self.end_policy {
            hftbacktest::backtest::result::EndPolicy::DrainAll => None,
            hftbacktest::backtest::result::EndPolicy::StopAtDataEnd => Some(self.data_end_ts),
            hftbacktest::backtest::result::EndPolicy::StopAtTime(timestamp) => Some(timestamp),
        };
    ...
    pub fn set_end_policy(&mut self, policy: hftbacktest::backtest::result::EndPolicy) {
    ...
    fn close_terminal_positions(&mut self) -> Result<(), MaterializedBarError> {
        ...
        for asset_no in 0..self.execution.positions().len() {
            ...
            self.execution
                .apply(&[hftbacktest::backtest::bar::BarMatchOutcome::Fill {
                    command,
    ...
    pub fn execution_reports(&self) -> &[hftbacktest::backtest::execution::ExecutionReport] {
        self.execution.reports()
    }

    pub fn funding_reports(&self) -> &[hftbacktest::backtest::execution::FundingReport] {
    ...
    pub fn configure_execution(
        &mut self,
        configs: Vec<hftbacktest::backtest::execution::SharedTickExecutionConfig>,
    ...
    where
        R: hftbacktest::backtest::execution::LocalPreTradeRisk + 'static,
    {
    ...
    where
        R: hftbacktest::backtest::execution::VenueRisk + 'static,
    {
    ...
    pub fn projected_execution_events(
        &self,
    ) -> &[(usize, hftbacktest::backtest::execution::ProjectedEvent)] {
            ...
            match decoded {
                Some(hftbacktest::backtest::execution::ExecutionCommand::Submit(request)) => {
                    ...
                    match self.execution.check_local_submit(command, ctx.now)? {
                        hftbacktest::backtest::execution::RiskDecision::Allow => {}
                        hftbacktest::backtest::execution::RiskDecision::Reject { reason } => {
                    ...
                    if self.contingencies.should_reject(command.order_id) {
                        self.execution.reject_local(
                            command,
                            ctx.now,
                            hftbacktest::backtest::execution::RiskReason::Custom(0xC001),
                        )?;
                        continue;
                    }
    ...
    fn match_at_next_open(&mut self, meta: BarBatchMeta) -> Result<(), MaterializedBarError> {
        ...
        for outcome in self.matcher.outcomes() {
            let command = match outcome {
                hftbacktest::backtest::bar::BarMatchOutcome::Fill { command, .. }
                | hftbacktest::backtest::bar::BarMatchOutcome::Expired { command, .. } => command,
            };
            self.gtd_orders
                .remove(&(command.asset_no, command.order_id));
        }
                ...
                match action {
                    ...
                    ConditionalAction::Expire { .. } => {
                        ...
                        self.execution.apply(&[
                            hftbacktest::backtest::bar::BarMatchOutcome::Expired {
                                command: pending.command,
    ...
    fn enqueue_risk_actions(&mut self, now: i64) -> Result<(), MaterializedBarError> {
        for action in self.execution.take_risk_actions() {
            match action {
                hftbacktest::backtest::execution::RiskAction::Cancel {
                    venue_id,
                    instrument_id,
                    order_id,
                    ..
                ...
                hftbacktest::backtest::execution::RiskAction::Liquidate {
                    venue_id,
                    instrument_id,
                    ..
                } => {
    ...
    fn process_next_expiry(&mut self) -> Result<bool, MaterializedBarError> {
        ...
        for ((asset_no, order_id), pending) in expired {
            ...
            self.execution
                .apply(&[hftbacktest::backtest::bar::BarMatchOutcome::Expired {
                    command: pending.command,
                    local_submit_ts: pending.local_submit_ts,
        ...
        loop {
            ...
            match self.next_boundary(market, bar_delivery) {
                ...
                Some(BarBoundary::MarketOpen) => {
                    ...
                    self.dispatch_platform_event(hftbacktest::backtest::scheduler::EventKey {
...
pub enum RuntimeError {
    ...
    Source {
        component: hftbacktest::backtest::result::EngineComponent,
        code: hftbacktest::backtest::result::EngineErrorCode,
        context: String,
    },
}
...
pub struct RuntimeRunStats {
    ...
    pub termination: hftbacktest::backtest::result::RunTermination,
}
...
impl Default for RuntimeRunStats {
    fn default() -> Self {
        Self {
            ...
            termination: hftbacktest::backtest::result::RunTermination::DataEnd,
        }
    }
}
    ...
    let result = match start_result {
        ...
        Ok(()) => loop {
            if ctx.stop_requested != 0 {
                stats.termination = hftbacktest::backtest::result::RunTermination::StrategyStop;
                break Ok(());
            }
...
pub fn run_event_runtime_scoped<S: RuntimeEventSource>(
    ...
    callbacks: &CallbackRegistry,
    ctx: &mut StrategyRuntimeContext,
) -> Result<RuntimeRunStats, hftbacktest::backtest::result::StructuredEngineError> {
...
pub struct PreparedBarRuntime {
    ...
    pub result_template: hftbacktest::backtest::result::BacktestResult,
    next_run_id: u64,
}
...
impl PreparedBarRuntime {
    pub fn new(
        ...
        result_template: hftbacktest::backtest::result::BacktestResult,
    ) -> Self {
...
impl hftbacktest::backtest::result::ReusableRuntime for PreparedBarRuntime {
    type Error = hftbacktest::backtest::result::StructuredEngineError;
    type Output = hftbacktest::backtest::result::BacktestResult;
    ...
    fn run_once(&mut self) -> Result<Self::Output, Self::Error> {
        ...
        let cpu_start = hftbacktest::backtest::result::process_cpu_time_ns();
        ...
        result.cpu_time_ns =
            hftbacktest::backtest::result::process_cpu_time_ns().saturating_sub(cpu_start);
        ...
        let counts = hftbacktest::backtest::result::execution_report_counts(reports);
    ...
    fn reset(&mut self) -> Result<(), Self::Error> {
        self.source.reset().map_err(|error| {
            hftbacktest::backtest::result::StructuredEngineError {
                run_id: self.next_run_id,
                component: hftbacktest::backtest::result::EngineComponent::DataSource,
                event_key: None,
                code: hftbacktest::backtest::result::EngineErrorCode::InvalidData,
                context: error.to_string(),
            }
        })?;
...
mod tests {
    ...
    fn reproducibility_metadata() -> ReproducibilityMetadata {
        ...
        ReproducibilityMetadata {
            ...
            phase_contract_version: hftbacktest::backtest::scheduler::PHASE_CONTRACT_VERSION,
    ...
    fn stop_at_data_end_does_not_drain_future_timer_or_funding() {
        ...
        let mut source = MaterializedBarSource::new(&records, 1).unwrap();
        source.set_end_policy(hftbacktest::backtest::result::EndPolicy::StopAtDataEnd);
        source.schedule_timer(RuntimeTimer {
            deadline_ts: 61,
            owner_id: 1,
            timer_id: 1,
    ...
    fn prepared_result_separates_exchange_and_delivery_time_and_reports_strategy_stop() {
        ...
        let result =
            hftbacktest::backtest::result::ReusableRuntime::run_once(&mut runtime).unwrap();
        assert_eq!(
            result.termination,
            hftbacktest::backtest::result::RunTermination::StrategyStop
        );
    ...
    fn cancel_command_decodes_without_submit_only_fields() {
        ...
        let decoded = decode_order_command(command, 100, VenueId(3), InstrumentId(4))
            .unwrap()
            .unwrap();
        let hftbacktest::backtest::execution::ExecutionCommand::Cancel(cancel) = decoded else {
    ...
    fn command_bridge_preserves_execution_algorithm_origin() {
        ...
        let decoded = decode_order_command(command, 7, VenueId(0), InstrumentId(1))
            .unwrap()
            .unwrap();
        let hftbacktest::backtest::execution::ExecutionCommand::Submit(request) = decoded else {
            panic!("expected submit");
        };
        assert_eq!(
            request.origin,
            hftbacktest::backtest::execution::OrderOrigin::ExecutionAlgorithm
        );
    }
    ...
    fn same_time_post_trade_action_does_not_precede_matching() {
        let mut scheduler = hftbacktest::backtest::scheduler::GlobalScheduler::new();
    ...
    impl ExecutionAlgorithm for EmitOnce {
        fn on_event(
            &mut self,
            key: hftbacktest::backtest::scheduler::EventKey,
            out: &mut Vec<hftbacktest::backtest::execution::ExecutionCommand>,
        ) {
            if self.emitted {
                return;
            }
            self.emitted = true;
            out.push(hftbacktest::backtest::execution::ExecutionCommand::Submit(
                hftbacktest::backtest::execution::ExecutionOrderRequest {
                    ...
                    time_in_force: hftbacktest::types::TimeInForce::IOC,
                    order_type: hftbacktest::types::OrdType::Market,
                    reduce_only: false,
                    origin: hftbacktest::backtest::execution::OrderOrigin::ExecutionAlgorithm,
    ...
    fn bracket_children_activate_then_oco_cancel_through_shared_execution() {
        ...
        assert!(source.register_contingency(ContingencyGroup {
            group_id: 7,
            kind: hftbacktest::backtest::platform::ContingencyKind::Bracket,
            venue_id: VenueId(0),
            instrument_id: InstrumentId(1),
            parent: Some(1),
            children: vec![2, 3],
        ...
        assert!(source.execution_reports().iter().any(|report| {
            report.order_id == 3
                && report.kind == hftbacktest::backtest::execution::ExecutionReportKind::Canceled
        }));
    }

    #[test]
    fn runtime_abi_descriptor_matches_the_rust_context() {
        let descriptor = runtime_abi_descriptor();
        assert!(descriptor.verify_fingerprint());
        assert_eq!(descriptor.abi_version, STRATEGY_ABI_VERSION);
        assert_eq!(descriptor.event_slot_count, EVENT_SLOT_COUNT as u64);
