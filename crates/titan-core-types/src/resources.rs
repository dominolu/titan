...
use crate::{ComponentIdentity, ComponentState, CoreError, ErrorKind};
...
/// Sole owner of a component instance's revocable resources.
...
impl ResourceScope {
    ...
    pub fn close(&mut self) -> Result<(), Vec<CoreError>> {
        let mut entries = {
...
impl ResourceScopeHandle {
    ...
    pub fn child(&self, name: impl Into<Arc<str>>) -> Result<ResourceScopeHandle, CoreError> {
...
pub struct ClosureResource<F: FnMut() -> Result<(), CoreError> + Send + 'static>(pub Option<F>);

impl<F: FnMut() -> Result<(), CoreError> + Send + 'static> Resource for ClosureResource<F> {
    fn close(&mut self) -> Result<(), CoreError> {
