...
- `EventReceiver` channels driven by service-owned subscriber threads, with callback
...
- authorized zero-copy MarketBatch reservations so connectors can encode directly into EventArena;
...
- `TradingRuntime` composition that starts EventEngine before core services and stops strategy,
  execution, account and market services before draining EventEngine.
